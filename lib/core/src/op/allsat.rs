...
use crate::lit::Lit;
use crate::solver::{SimpleSolver, SolveResponse, Solver};
...
impl<S> SimpleAllSat for S where S: SimpleSolver {}

pub trait SimpleAllSat: SimpleSolver + Sized {
    fn all_sat<T, F>(&mut self, f: F) -> SimpleAllSolutionsIter<Self, F>
    where
        F: FnMut(&mut Self) -> T,
    {
        // If no essential vars were passed, then *all* variables are essential!
    ...
    fn all_sat_essential<I, T, F>(&mut self, essential: I, f: F) -> SimpleAllSolutionsIter<Self, F>
    where
        I: IntoIterator,
        I::Item: Into<Lit>,
        F: FnMut(&mut Self) -> T,
    {
        SimpleAllSolutionsIter::new(self, f, essential)
    }
...
pub struct SimpleAllSolutionsIter<'s, S, F>
where
    S: SimpleSolver,
{
    solver: &'s mut S,
...
impl<'s, S, F> SimpleAllSolutionsIter<'s, S, F>
where
    S: SimpleSolver,
{
    fn new<I>(solver: &'s mut S, callback: F, essential: I) -> Self
...
impl<'s, T, S, F> Iterator for SimpleAllSolutionsIter<'s, S, F>
where
    S: SimpleSolver,
    F: FnMut(&mut S) -> T,
{
